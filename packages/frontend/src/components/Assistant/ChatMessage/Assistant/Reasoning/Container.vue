...
import { useAutoScroll } from "@/components/Assistant/useAutoScroll";
...
<template>
  <div class="py-1">
    <template v-if="isReasoning">
      ...
      >
        <div class="text-surface-500 [&_*]:text-surface-500 text-sm">
    ...
    <template v-else>
      <button
        v-if="hasContent"
        class="group w-full flex items-center text-surface-500 hover:text-surface-400 text-sm transition-colors font-mono cursor-pointer"
      ...
      <div v-else class="flex items-center text-surface-500 text-sm font-mono">
        <span>{{ reasoningText }}</span>
      </div>
      <div
